namespace PaperPlaneTools.AR {
    ...
    public class MainScript: WebCamera {
        ...
        Vector3 posAntiguaMando;
        Quaternion rotAntiguaMando;
			...
			while (index >= 0) {
				...
				if (markerOnScene.bestMatchIndex < 0) {
					if (markerOnScene.destroyAt < 0) {
                        ...
                        if (markerOnScene.gameObject.name.Equals("MandoCapsula(Clone)"))
                        {
                            inicializado = false;
                        }
		...
		private void PositionObject(GameObject gameObject, Matrix4x4 transformMatrix) {
            ...
            if (gameObject.name.Equals("MandoCapsula(Clone)"))
            {
                ...
                } else
                {
                    ...
                    Quaternion primeraInterpolacion = Quaternion.Slerp(gameObject.transform.localRotation, rotAntiguaMando, 0.5f);

                    Vector3 newPos = (posMando + gameObject.transform.localPosition + posAntiguaMando) / 3;
                    Quaternion newRot = Quaternion.Slerp(primeraInterpolacion, rotMando, 0.5f);
